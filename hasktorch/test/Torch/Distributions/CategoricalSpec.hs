...
spec = do
  ...
  it "logProb" $ do
    ...
    F.allclose (toDynamic t) (D.asTensor [-0.22314355, -0.22314355 :: Float]) 0.0001 0.0001 False `shouldBe` True

  it "entropy" $ do
    -- putStrLn . show $ entropy d
    let t :: Tnsr 'D.Float '[] = UnsafeMkTensor $ entropy d
    abs (toFloat t - 0.5004017) < 0.01 `shouldBe` True
    F.allclose (toDynamic t) (D.asTensor [0.5004017 :: Float]) 0.001 0.001 False `shouldBe` True
