...
spec = do
  ...
  it "logProb" $ do
    ...
    F.allclose (toDynamic t) (D.asTensor [-0.6547297, -0.9870405 :: Float]) 0.001 0.001 False `shouldBe` True

  it "entropy" $ do
    -- putStrLn . show $ entropy d
    let t :: Tnsr 'D.Float '[2] = UnsafeMkTensor $ entropy d
    F.allclose (toDynamic t) (D.asTensor [0.7663015, 0.5042092 :: Float]) 0.0001 0.0001 False `shouldBe` True
