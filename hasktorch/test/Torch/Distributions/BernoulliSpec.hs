...
spec = do
  ...
  it "logProb" $ do
    ...
    -- (D.shape $ toDynamic t) `shouldBe` (D.shape $ D.asTensor [-0.6749387 :: Float])
    -- toList (Just $ toDynamic t) `shouldBe` [[-0.6749387 :: Float]]
    D.asValue (toDynamic t) `shouldBe` [[-0.6749387 :: Float]]
    -- F.allclose (toDynamic t) (D.asTensor [[-0.6749387 :: Float]]) 0.001 0.001 False `shouldBe` True
