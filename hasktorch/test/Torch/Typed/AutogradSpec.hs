...
data
  RastriginStack
    ...
    (ns :: [Nat])
    (dtypes :: [D.DType])
    (devices :: [(D.DeviceType, Nat)]) where
  Rastrigin1 ::
    forall n dtype device.
    RastriginLayer n dtype device ->
...
instance
  ...
  GParameterized (K1 R (RastriginStack 1 '[n] '[dtype] '[device])) parameters
  where
...
instance
  ...
  GParameterized (K1 R (RastriginStack num (n ': ns) (dtype ': dtypes) (device ': devices))) parameters''
  where
...
instance
  ...
  Apply' (RastriginA a (Proxy device') (Proxy dtype')) (Parameter device dtype '[n]) (Tensor device' dtype' '[])
  where
...
instance
  ...
  Apply' GradientsTestInner ((Tensor device dtype shape, Tensor device dtype shape), IO ()) (IO ())
  where
...
instance
  ...
  Apply' (GradientsTestOuter a) (((Proxy device, Proxy dtype), RastriginSpec num ns dtypes devices), IO ()) (IO ())
  where
...
spec = describe "grad" $ do
  ...
  when
    ( elem (D.Device {D.deviceType = D.CPU, D.deviceIndex = 0}) availableDevices
