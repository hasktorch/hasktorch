...
data
  ...
  TransformerDecoder ::
    ...
    { -- | decoder layer stack
      ...
      -- | decoder layer norm
      tdLayerNorm :: LayerNorm device dataType ( 'Shape '[decoderInputEmbedDim]),
      -- | decoder dropout
...
type HasInitializeTransformerDecoderC numLayers device dataType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim dropoutP =
  ( HasInitializeTransformerDecoderStackC numLayers device dataType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim dropoutP,
    HasInitializeTransformerDecoderStack
      ...
      ffnDim
      dropoutP,
    HasInitializeLayerNormC device dataType ( 'Shape '[decoderInputEmbedDim]),
    Scalar dropoutP,
      ...
      go deviceType dType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim dropoutP eps = runState $ do
        ...
        let layerNorm =
              withoutShape @( 'Shape '[decoderInputEmbedDim])
                ( withoutDataType @dataType
                    ( withoutDevice @device
                        ( initialize @(LayerNorm device dataType ( 'Shape '[decoderInputEmbedDim]))
                        )
                        deviceType
                    )
                    dType
...
type HasForwardTransformerDecoderC
  ...
  ( Scalar dropoutP,
    HasForwardTransformerDecoderStack (1 <=? numLayers) numLayers device dataType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim dropoutP requiresGradient decoderInputLayout (decoderInputDevice <+> generatorDevice) decoderInputDataType decoderInputShape encoderOutputLayout encoderOutputDevice encoderOutputDataType encoderOutputShape decoderAttentionMaskLayout decoderAttentionMaskDevice decoderAttentionMaskDataType decoderAttentionMaskShape crossAttentionMaskLayout crossAttentionMaskDevice crossAttentionMaskDataType crossAttentionMaskShape (decoderInputDevice <+> generatorDevice),
    HasForward
      (LayerNorm device dataType ( 'Shape '[decoderInputEmbedDim]))
      (HasForwardTransformerDecoderStackOutput (1 <=? numLayers) numLayers device dataType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim requiresGradient decoderInputLayout (decoderInputDevice <+> generatorDevice) decoderInputDataType decoderInputShape encoderOutputLayout encoderOutputDevice encoderOutputDataType encoderOutputShape decoderAttentionMaskLayout decoderAttentionMaskDevice decoderAttentionMaskDataType decoderAttentionMaskShape crossAttentionMaskLayout crossAttentionMaskDevice crossAttentionMaskDataType crossAttentionMaskShape  (decoderInputDevice <+> generatorDevice))
      (HasForwardTransformerDecoderStackGeneratorOutput (1 <=? numLayers) numLayers device (decoderInputDevice <+> generatorDevice) encoderOutputDevice decoderAttentionMaskDevice crossAttentionMaskDevice (decoderInputDevice <+> generatorDevice)),
    HasForward
      (Dropout dropoutP)
      ( ForwardOutput
          (LayerNorm device dataType ( 'Shape '[decoderInputEmbedDim]))
...
instance
  HasForwardTransformerDecoderC numLayers device dataType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim dropoutP requiresGradient decoderInputLayout decoderInputDevice decoderInputDataType decoderInputShape encoderOutputLayout encoderOutputDevice encoderOutputDataType encoderOutputShape decoderAttentionMaskLayout decoderAttentionMaskDevice decoderAttentionMaskDataType decoderAttentionMaskShape crossAttentionMaskLayout crossAttentionMaskDevice crossAttentionMaskDataType crossAttentionMaskShape generatorDevice =>
  HasForward (TransformerDecoder numLayers device dataType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim dropoutP) (Tensor requiresGradient decoderInputLayout decoderInputDevice decoderInputDataType decoderInputShape, Tensor requiresGradient encoderOutputLayout encoderOutputDevice encoderOutputDataType encoderOutputShape, Tensor requiresGradient decoderAttentionMaskLayout decoderAttentionMaskDevice decoderAttentionMaskDataType decoderAttentionMaskShape, Tensor requiresGradient crossAttentionMaskLayout crossAttentionMaskDevice crossAttentionMaskDataType crossAttentionMaskShape) (Generator generatorDevice)
  where
  type
    ForwardOutput (TransformerDecoder numLayers device dataType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim dropoutP) (Tensor requiresGradient decoderInputLayout decoderInputDevice decoderInputDataType decoderInputShape, Tensor requiresGradient encoderOutputLayout encoderOutputDevice encoderOutputDataType encoderOutputShape, Tensor requiresGradient decoderAttentionMaskLayout decoderAttentionMaskDevice decoderAttentionMaskDataType decoderAttentionMaskShape, Tensor requiresGradient crossAttentionMaskLayout crossAttentionMaskDevice crossAttentionMaskDataType crossAttentionMaskShape) (Generator generatorDevice) =
      ForwardOutput
        (Dropout dropoutP)
        ( ForwardOutput
            (LayerNorm device dataType ( 'Shape '[decoderInputEmbedDim]))
  ...
  type
    ForwardGeneratorOutput (TransformerDecoder numLayers device dataType headDim headEmbedDim embedDim decoderInputEmbedDim encoderOutputEmbedDim ffnDim dropoutP) (Tensor requiresGradient decoderInputLayout decoderInputDevice decoderInputDataType decoderInputShape, Tensor requiresGradient encoderOutputLayout encoderOutputDevice encoderOutputDataType encoderOutputShape, Tensor requiresGradient decoderAttentionMaskLayout decoderAttentionMaskDevice decoderAttentionMaskDataType decoderAttentionMaskShape, Tensor requiresGradient crossAttentionMaskLayout crossAttentionMaskDevice crossAttentionMaskDataType crossAttentionMaskShape) (Generator generatorDevice) =
      ForwardGeneratorOutput
        (Dropout dropoutP)
        ( ForwardOutput
            (LayerNorm device dataType ( 'Shape '[decoderInputEmbedDim]))
