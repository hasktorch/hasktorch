...
renderTenTypeSpec = do
  ...
  it "renders all non-THC prefixes as the library appended to the raw tensor type" . propertyST (\p -> not (isTHC p || isReal p)) $
    \tt@(Pair (raw, l)) ->
      renderTenType tt `shouldBe` (tshow l <> tshow raw)

  it "renders THC as THCuda for concrete tensor types" . propertyST (\p -> isTHC p && not (isReal p)) $
    \tt@(Pair (raw, _)) ->
 ...
 where
  propertyST :: Arbitrary a => Show a => Testable prop => (a -> Bool) -> (a -> prop) -> Property
