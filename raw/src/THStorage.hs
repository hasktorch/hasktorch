...
foreign import ccall "THStorage.h &THLongStorage_inferSize2"
  p_THLongStorage_inferSize2 :: FunPtr (Ptr CTHLongStorage -> Ptr CLLong -> CLLong -> Ptr CLLong -> CLLong -> Ptr CChar -> CInt -> CInt)

-- |p_THLongStorage_inferSizeN : Pointer to function : output n sizes dims error_buffer buffer_len -> int
foreign import ccall "THStorage.h &THLongStorage_inferSizeN"
  p_THLongStorage_inferSizeN :: FunPtr (Ptr CTHLongStorage -> CInt -> Ptr (Ptr CLLong) -> Ptr CLLong -> Ptr CChar -> CInt -> CInt)
...
foreign import ccall "THStorage.h &THLongStorage_inferExpandGeometry"
  p_THLongStorage_inferExpandGeometry :: FunPtr (Ptr CLLong -> Ptr CLLong -> CLLong -> Ptr CTHLongStorage -> Ptr (Ptr CLLong) -> Ptr (Ptr CLLong) -> Ptr CChar -> CInt -> CInt)