...
foreign import ccall "THCTensor.h THCShortTensor_newExpand"
  c_newExpand :: Ptr C'THCState -> Ptr C'THCudaShortTensor -> Ptr C'THCLongStorage -> IO (Ptr C'THCudaShortTensor)

-- | c_expand :  state r tensor sizes -> void
foreign import ccall "THCTensor.h THCShortTensor_expand"
  c_expand :: Ptr C'THCState -> Ptr C'THCudaShortTensor -> Ptr C'THCudaShortTensor -> Ptr C'THCLongStorage -> IO ()
...
foreign import ccall "THCTensor.h &THCShortTensor_newExpand"
  p_newExpand :: FunPtr (Ptr C'THCState -> Ptr C'THCudaShortTensor -> Ptr C'THCLongStorage -> IO (Ptr C'THCudaShortTensor))

-- | p_expand : Pointer to function : state r tensor sizes -> void
foreign import ccall "THCTensor.h &THCShortTensor_expand"
  p_expand :: FunPtr (Ptr C'THCState -> Ptr C'THCudaShortTensor -> Ptr C'THCudaShortTensor -> Ptr C'THCLongStorage -> IO ())
