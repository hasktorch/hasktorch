...
foreign import ccall "THCTensorRandom.h THCFloatTensor_rand"
  c_rand :: Ptr C'THCState -> Ptr C'THCudaFloatTensor -> Ptr C'THCLongStorage -> IO ()

-- | c_randn :  state r_ size -> void
foreign import ccall "THCTensorRandom.h THCFloatTensor_randn"
  c_randn :: Ptr C'THCState -> Ptr C'THCudaFloatTensor -> Ptr C'THCLongStorage -> IO ()
...
foreign import ccall "THCTensorRandom.h &THCFloatTensor_rand"
  p_rand :: FunPtr (Ptr C'THCState -> Ptr C'THCudaFloatTensor -> Ptr C'THCLongStorage -> IO ())

-- | p_randn : Pointer to function : state r_ size -> void
foreign import ccall "THCTensorRandom.h &THCFloatTensor_randn"
  p_randn :: FunPtr (Ptr C'THCState -> Ptr C'THCudaFloatTensor -> Ptr C'THCLongStorage -> IO ())
