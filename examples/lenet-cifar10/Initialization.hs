  ...
  where
    ...
    runEpoch val e net
      ...
      | otherwise = do
        ...
        (net', train) <- runBatches estr bsz lr t0 e ltrain net
        testNet net' val
        report net0 val
...
trainStep lr net xs ys =
  first (LeNet.update net lr) <$> backward lr net xs ys
...
report net ltest = do
  ...
  -- (_, loss) <- backward lr net (snd <$> test) (fst <$> test)
