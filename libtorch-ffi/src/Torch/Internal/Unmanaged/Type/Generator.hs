...
return new at::Generator(at::detail::getCUDAHooks().getCUDADefaultGenerator($(uint16_t _device_index)));
#else
...
getDefaultMPSGenerator =
  [C.throwBlock| at::Generator* {
#if TORCH_VERSION_MAJOR == 2 && TORCH_VERSION_MINOR < 7
return new at::Generator(at::detail::getMPSHooks().getMPSDefaultGenerator());
#else
