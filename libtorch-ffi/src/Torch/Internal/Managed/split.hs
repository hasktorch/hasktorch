...
genHeader ls idx = map (\l -> if l =~ "module Torch.Internal.Managed.Native where" then "module Torch.Internal.Managed.Native.Native" <> show idx <> " where" else l) ls

splitFunctions :: [String] -> [String] -> [[String]]
splitFunctions [] buf = [reverse buf]
