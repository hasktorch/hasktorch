...
genericP :: (GenericOps' t, Show (HaskType' t)) => t -> IO ()
genericP t = withForeignPtr (getForeign t) Gen.dispRaw

genericToList :: GenericOps' t => (HaskType' t -> Item t) -> t -> [Item t]
...
genericFromListNd
  :: (GenericMath' t, Num (HaskType' t), k ~ Nat)
  => (Item t -> HaskType' t) -> Dim (d::[k]) -> [Item t] -> t
...
genericFromList1d
  :: forall t k . (GenericMath' t, Num (HaskType' t), k ~ Nat)
  => (Item t -> HaskType' t) -> [Item t] -> t
genericFromList1d translate l = unsafePerformIO $ do
 ...
 where
  ...
  mutTensor t (idx, value) = withForeignPtr t $ \tp -> Gen.set1d tp (fromIntegral idx) (translate value)
{-# NOINLINE genericFromList1d #-}

-- |Copy contents of tensor into a new one of specified size
genericResize
  :: forall t d . (GenericMath' t, Num (HaskType' t), k ~ Nat)
  => t -> Dim (d::[k]) -> t
genericResize t d = unsafePerformIO $ do
  let resDummy = genericNew d :: t
  newPtr <- withForeignPtr (getForeign t) Gen.newClone
  newFPtr <- newForeignPtr Gen._free newPtr
  with2ForeignPtrs newFPtr (getForeign resDummy) Gen.resizeAs
...
genericGet :: (GenericOps' t, k ~ Nat) => (HaskType (TensorPtrType t) -> Item t) -> Dim (d::[k]) -> t -> Item t
...
genericNewWithTensor t = unsafePerformIO $ do
  newPtr <- withForeignPtr (getForeign t) Gen.newWithTensor
  newFPtr <- newForeignPtr Gen._free newPtr
  pure $ construct newFPtr
...
genericInit :: (GenericMath' t, k ~ Nat) => (Item t -> HaskType' t) -> Dim (d::[k]) -> Item t -> t
...
genericTranspose (fromIntegral->dim1C) (fromIntegral->dim2C) t = unsafePerformIO $ do
  newPtr <- withForeignPtr (getForeign t) (\p -> Gen.newTranspose p dim1C dim2C)
  newFPtr <- newForeignPtr Gen._free newPtr
  pure $ construct newFPtr
...
genericTrans t = unsafePerformIO $ do
  newPtr <- withForeignPtr (getForeign t) (\p -> Gen.newTranspose p 1 0)
  newFPtr <- newForeignPtr Gen._free newPtr
  pure $ construct newFPtr
