...
_bernoulli_DoubleTensor t d =
  with2DynamicState t d Sig.c_bernoulli_DoubleTensor
...
_normal_stddevs t a b = with2DynamicState t b
  $ \s' t' b' -> Sig.c_normal_stddevs s' t' (hs2cAccReal a) b'

-- | CUDA version of 'THRandom._normal_means_stddevs'
_normal_means_stddevs :: Dynamic -> Dynamic -> Dynamic -> IO ()
_normal_means_stddevs t t0 t1 =
  with3DynamicState t t0 t1 Sig.c_normal_means_stddevs
...
_multinomialAliasSetup r l t =
  runManaged
    .   (liftIO =<<)
    $   Sig.c_multinomialAliasSetup
    <$> managedState
    <*> managedTensor r
...
_multinomialAliasDraw r l t =
  runManaged
    .   (liftIO =<<)
    $   Sig.c_multinomialAliasDraw
    <$> managedState
    <*> managed (withForeignPtr . snd . Sig.longDynamicState $ r)
    <*> managed (withForeignPtr . snd . Sig.longDynamicState $ l)
    <*> managedTensor t
