...
, haskellCompiler ? "ghc883"
  ...
  hsPkgs = 
    { overlays
    , pkgs ? (import nixpkgsSrc (haskellNix.nixpkgsArgs // { overlays = haskellNix.overlays ++ overlays; }))
    }: pkgs.haskell-nix.cabalProject {
      src = pkgs.haskell-nix.haskellLib.cleanGit { name = "hasktorch"; src = ./.; };
      compiler-nix-name = haskellCompiler;
      modules = [
        ({ config, ... }: {
          packages.libtorch-ffi.configureFlags = [ "--extra-include-dirs=${pkgs.torch}/include/torch/csrc/api/include" ];
        })
      ];
    };

  hsPkgsCpu = hsPkgs { overlays = [ libtorchOverlayCpu ]; };
  hsPkgsCuda92 = hsPkgs { overlays =  [ libtorchOverlayCuda92 ]; };
  hsPkgsCuda102 = hsPkgs { overlays =  [ libtorchOverlayCuda102 ]; };
in
{
