...
genWellTypedExp'' :: forall a. Eq a => STLC.Ty -> GTyM a (STLC.Exp a)
genWellTypedExp'' (STLC.Arr ty' ty'') = do
  uname <- fresh
  STLC.lam ty' uname <$> local (insertVar uname ty') (genWellTypedExp' ty'')
genWellTypedExp'' STLC.Nat = STLC.nat <$> Gen.int (Range.linear 0 10)

genWellTypedExp''' :: forall a. Eq a => STLC.Ty -> GTyM a (STLC.Exp a)
genWellTypedExp''' STLC.Nat = STLC.Succ <$> genWellTypedExp' STLC.Nat
