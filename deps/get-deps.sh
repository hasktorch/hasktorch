...
echo  "Note: run this script using `source ./get-deps.sh` to set LD_LIBRARY_PATH\n\n"
...
case "$(uname)" in
  "Darwin")
    ...
    export LD_LIBRARY_PATH=$(pwd)/mklml_mac_2019.0.1.20181227/lib:$(pwd)/libtorch/lib
    ;;
  "Linux")
    ...
    export LD_LIBRARY_PATH=$(pwd)/mklml_lnx_2019.0.1.20181227/lib:$(pwd)/libtorch/lib
    ;;
esac
