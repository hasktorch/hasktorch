...
module Torch.Core.Tensor.Static.Generic
  ( THSTensor'
  , THSTensorMath'
  , DownCastValueFn
...
import qualified Torch.Core.Tensor.Types as Dyn (THForeignRef(..))
...
type instance THForeignType (TensorDoubleStatic dim) = CTHDoubleTensor
...
newtype TensorFloatStatic (ds :: [Nat])
  = TFS { tfsTensor :: ForeignPtr CTHFloatTensor }
  deriving (Show)

type TFS = TensorFloatStatic
type instance THForeignType (TensorFloatStatic (dim :: [Nat])) = CTHFloatTensor
...
_withManaged3
  ...
  => (Ptr (THForeignType (t a)) -> Ptr (THForeignType (t b)) -> Ptr CTHLongStorage -> IO ())
